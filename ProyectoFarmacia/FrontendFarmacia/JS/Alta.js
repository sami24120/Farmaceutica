function getPatients() {
	...
	http.open(
		"GET",
		"http://localhost:8080/backendfarmacia/ServePatients?mail=" +
			mail +
			"&session=" +
			session,
		true
...
function getMedicines() {
	...
	http.open( "GET", "http://localhost:8080/backendfarmacia/ServMedicines?mail=" +	mail +"&session=" + session, true);
...
function enviar() {
  ...
  http.open("POST", "http://localhost:8080/backendfarmacia/Release", true);
  http.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
  console.log("la conexión  esta bien");
  ...
  http.send( "mail=" +    encodeURIComponent(mail) +    "&session=" +    encodeURIComponent(session) +    "&idXip=" +   encodeURIComponent(idXip) +    "&mailP=" +    encodeURIComponent(mailP) +    "&idMed=" +    encodeURIComponent(idMed) +    "&date=" +    encodeURIComponent(date)  );
}
